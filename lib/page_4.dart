...
void initState() {
 ...
 void getData() async {
   try{
     isLoading = true;
     Response response = await dio.get('https://api.asocial.chat/api/v1/auth/intrests-list');
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 30),
            Expanded(
              child: data.body == null ? Center(child: CircularProgressIndicator(),) :
              ListView.builder(
                itemCount: data.body?.length,
                padding: EdgeInsets.zero,
                itemBuilder: (context, index) {
                return _buildCategorySection(data.body?[index].name ?? "", data.body?[index].intrestList ?? [], selectedCreativityIndices);
              },),
            ),
            Center(
              child: Row(
  ...
  Widget _buildCategorySection(String title, List<IntrestList> items,
